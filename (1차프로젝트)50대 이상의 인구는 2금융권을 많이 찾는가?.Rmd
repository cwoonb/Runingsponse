...
<span style="color:red">   **참고사항:**  KCB 금융 데이터의 모든 수치는 연령, 기간, 성별, 지역별 평균수치로 구성되어있다. </span> 
```{r}
# 데이터 기본 전처리
...
## 데이터 분석 및 시각화 <br> {.tabset .tabset-fade}
...
suppressMessages(
  ...
  summarize(region_loan = median(monthly_bk_loan)))) -> loan_median
...
suppressMessages(
  suppressWarnings(
    ...
    group_by(city) %>%
    filter(str_detect(ages, '^[5-6]')) %>%
    summarize(region_loan2 = median(monthly_2nd_loan)))) -> loan_median2


# 조인 기준 열 2금융

loan_median2 %>%
  mutate(id = case_when(loan_median2$city == '서울' ~ 0,
                        loan_median2$city == '부산' ~ 1,
                        loan_median2$city == '대구' ~ 2,
                        loan_median2$city == '인천' ~ 3,
                        loan_median2$city == '광주' ~ 4,
                        loan_median2$city == '대전' ~ 5,
                        loan_median2$city == '울산' ~ 6,
                        loan_median2$city == '경기' ~ 8,
                        loan_median2$city == '강원' ~ 9,
                        loan_median2$city == '충북' ~ 10,
                        loan_median2$city == '충남' ~ 11,
                        loan_median2$city == '전북' ~ 12,
                        loan_median2$city == '전남' ~ 13,
                        loan_median2$city == '경북' ~ 14,
                        loan_median2$city == '경남' ~ 15,
                        loan_median2$city == '제주' ~ 16,
  ))  -> loan_median2

# 지도 & 2금융대출량 데이터 조인
merge_result2 = left_join(korea_map, loan_median2, by = 'id')
merge_result2[merge_result2$id == 7,]$city = '세종'


# 지역별 대출량 지도
loan_map2 = suppressMessages(
  suppressWarnings(ggplot(data = merge_result2,
                  mapping = aes(x = long,
                                y = lat,
                                group = group,
                                fill = region_loan2,
                                text = paste('</br> 지역: ',
                                             city,
                                             ifelse(id != 7,
                                                    paste('</br> 대출량: ', region_loan2, '만원'),
                                                    paste('</br> 대출량: ', region_loan2))))) + geom_polygon() +
  coord_equal() +
  theme(axis.line = element_blank(),
        axis.text = element_blank(),
        axis.title = element_blank(),
  ...
  labs(fill = '2금융권 대출량')))
suppressMessages(
  suppressWarnings(ggplotly(loan_map2)))
```
...
region %>%
  filter(ages == '50대') %>%
  select(city, monthly_cd_loan,
         monthly_installments_loan,
         monthly_insurance_loan,
         monthly_sbk_loan) %>%
  group_by(city) %>%
  summarize_all(median) %>%
  gather(key = 'loan', value = 'median', -city))) -> loan_2nd_med50
# 2금융 합
suppressMessages(
  suppressWarnings(
loan_2nd_med50 %>%
...
suppressMessages(
  suppressWarnings(
ggplot(loan_2nd_med50, aes(x = city, y = median, fill = loan)) +
  geom_bar(stat = 'identity', position = 'stack', width = 0.7) +
  geom_text(inherit.aes = F,
            data = med_sum,
            mapping = aes(x = city,
            ...
            vjust = -0.3,
            size = 2.4) +
  labs(title = '50대 나이대 별 2금융 종류별 중위수',
       y = '대출량 (만원)',
       fill = '2금융권') +
  scale_fill_discrete(name = '2금융권',
                      labels = c('카드대출',
...
region %>%
  ...
  group_by(city) %>%
  summarize_all(median) %>%
  gather(key = 'loan', value = 'median', -city))) -> loan_2nd_med60
# 2금융 합
suppressMessages(
  suppressWarnings(
loan_2nd_med60 %>%
...
suppressMessages(
  suppressWarnings(
ggplot(loan_2nd_med60, aes(x = city, y = median, fill = loan)) +
  geom_bar(stat = 'identity', position = 'stack', width = 0.7) +
  geom_text(inherit.aes = F,
            data = med_sum,
            mapping = aes(x = city,
            ...
            vjust = -0.3,
            size = 2.4) +
  labs(title = '60대 나이대 별 2금융 종류별 중위수',
       y = '대출량 (만원)',
       fill = '2금융권') +
  scale_fill_discrete(name = '2금융권',
                      labels = c('카드대출',
...
<!-- 5. 지도 지역별 총 대출량만을 보고  지역별 차이가 있다고 볼 수없다. -->
<!--     지역별 2금융권을 봐야된다. -->


<!-- 5.3 도 역시 2금융 권에 대해 좀거 근거가 될만한 자료 제시  -->



<!-- 5.6 에서 WILCOXON은 작은 데이터에서 큰데이터를 비교할때 기각 할 수 있는지 활용하는것이다. -->



<!-- 지도에서 총 대출량이 다르다고 해서 2금융권의 대출량이 차이가 나야하는 이유가 있는가? -->
<!-- 2금융권의 퍼센트가 차이가 크게 나지 않는다고 하였는데, 무슨 기준으로 차이가 나지 않는다고 한것인가? -->
<!-- 가설검정에서 비모수기법을 사용할때, 또 하나의 단점은 가설검정 결과가 더 큰 모집단을 대표하기가 어려울 수 있다는 점이다. 이것의 문제는 없는가? -->
